...
impl Display for Notification {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        if self.status.to_uppercase().eq('CREATED) {
            return write!(f,
                "Hello {}, let's try ur new {} product: {}, only in BambangShop! Check it out: {}",
            ...
            return wite!(f,
                "Hello {}, we informed that our {} product calld {} already sold out...",
            ...
            return write!(f,
                ...
                elf.subscriber_name, self.product_type.to_lowercase(), self.product_title, self.product_url);
        }
    }
}